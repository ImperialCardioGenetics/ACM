...
# DCM_demographics & HVOL_demographics : the infiles for these won't be shared with the world. Would it be worth seperating the code this it? So at least the script can run smoothly
# NYHA CLASS is categorical - currently treated as continuous variable 
# Multiple Tables : Number in each group in column names is hard coded. Need to be fixed 
...
GroupTable<-GroupTable[,c("AGE_AT_scan","EF_echo","EF_MRI","DD_echo","DD_MRI","Sex_males","Ethnicity")]
...
ACM_DCM_summary<-Summary_burden_Analysis(ACM_DCM_data,"ACM vs DCM")
ACM_HVOL_summary<-Summary_burden_Analysis(ACM_HVOL_data, "ACM vs HVOL")
DCM_HVOL_summary<-Summary_burden_Analysis(DCM_HVOL_data, "DCM vs HVOL")
...
#ACM_DCM<-All_burden_Analysis(ACM_DCM_data,"ACM vs DCM")
#ACM_HVOL<-All_burden_Analysis(ACM_HVOL_data, "ACM vs HVOL")
#DCM_HVOL<-All_burden_Analysis(DCM_HVOL_data, "DCM vs HVOL")
##ACM & HVOL do not have any variants in some groups. So burden test result is NA
#BurdenTest_table_All <-t(RBIND(list(Mean_stdDev,ACM_DCM, ACM_HVOL,DCM_HVOL)))
#colnames(BurdenTest_table_All) <- c("ACM <br> (n=141)","DCM <br> (n=366)","HVOL <br> (n=445)","ACM vs DCM","ACM vs HVOL","DCM vs HVOL")


SuppTable_PAV_freq<-t(Mean_stdDev)
colnames(SuppTable_PAV_freq) <- c("ACM <br> (n=141)","DCM <br> (n=366)","HVOL <br> (n=445)","ExAC <br> (n=60706)")
...
<!-- Advice from James to make the code less redundant and reduce human error  

output <- RareVarRaw %>%
  tbl_df %>%
  mutate(new=paste(GENE,CSQ,sep="")) %>%
  select(CHROM,new) %>%
  # group_by() %>%
  # summarise() %>%
  outer_join(sample_id)

t<-RareVarRaw %>%
group_by(GENE,group,Var_Group)%>%
  summarise(n()) %>%
  right_join(SampleIDs)
